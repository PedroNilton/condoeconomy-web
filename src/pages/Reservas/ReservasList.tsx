    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-gray-500 dark:text-gray-400 mt-1">Controle os salões e a entrada de convidados no dia.</p>
        </div>
        <div className="flex items-center gap-2">
          <label className="text-gray-600 dark:text-gray-300 font-medium">Data:</label>
          <input 
            type="date" 
            value={dataSelecionada}
            onChange={(e) => setDataSelecionada(e.target.value)}
            className="px-4 py-2 border border-gray-300 dark:border-gray-600 rounded-lg outline-none focus:ring-2 focus:ring-blue-600"
          />
        </div>
      </div>
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-1 gap-6 overflow-x-auto">
          {reservas.map(reserva => (
            <div key={reserva.id} className="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 shadow-sm overflow-hidden">
              <div className="bg-gray-50 dark:bg-gray-900 px-6 py-4 border-b border-gray-200 dark:border-gray-700 flex justify-between items-center">
                <div>
                  <h4 className="font-semibold text-lg text-gray-800 dark:text-gray-100">{reserva.titulo}</h4>
              ...
              <div className="p-6">
                <h5 className="font-medium text-gray-800 dark:text-gray-100 flex items-center gap-2 mb-4">
                  <Users className="w-5 h-5 text-gray-400" /> Lista de Convidados ({reserva.convidados.length})
                </h5>
                
                {reserva.convidados.length === 0 ? (
                  <p className="text-gray-500 dark:text-gray-400 text-sm italic">Nenhum convidado listado para este evento.</p>
                ) : (
                  <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                    {reserva.convidados.map(convidado => (
                      <div key={convidado.id} className={`flex items-center justify-between p-3 rounded-lg border ${convidado.statusEntrada === 'ENTROU' ? 'bg-gray-50 dark:bg-gray-900 border-gray-200 dark:border-gray-700' : 'bg-white dark:bg-gray-800 border-gray-200 dark:border-gray-700 shadow-sm'}`}>
                        <div>
                          <p className="font-medium text-gray-800 dark:text-gray-100">{convidado.nome}</p>
                          <p className="text-xs text-gray-500 dark:text-gray-400">Doc: {convidado.documento || 'N/A'}</p>
